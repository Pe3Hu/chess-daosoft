...
func place_on_tile(tile_: Tile) -> void:
	if tile_.resource == resource.tile:
	...
	if move_resource != null:
		...
		match move_resource.type:
			...
			FrameworkSettings.MoveType.CASTLING:
				complement_castling_move(move_resource.castling_rook, is_canceling_move)
...
func capture(source_piece_: Piece = null, is_admin_: bool = false) -> void:
	if !is_admin_:
		if resource.player.spy_move == null and board.game.resource.current_mod == FrameworkSettings.ModeType.SPY: return 
...
func complement_castling_move(castling_rook_: PieceResource, is_canceled_: bool = false) -> void:
	if is_canceled_: return
	var rook_piece = board.get_piece(castling_rook_)
...
func consequence_of_placement() -> void:
	board.game.referee.apply_mods()
	var is_passing_initiative = board.game.referee.resource.active_player.is_last_initiative()
	var initiative = board.game.referee.resource.active_player.get_initiative()
	#var b = board.game.referee.resource.active_player.initiatives
	#var c = board.game.referee.resource.active_player.initiative_index
